...
poly_coef_x = MinimumSnapQPSolver(path(:, 1), ts, n_seg, n_order,t_order);
poly_coef_y = MinimumSnapQPSolver(path(:, 2), ts, n_seg, n_order,t_order);
...
function poly_coef = MinimumSnapQPSolver(waypoints, ts, n_seg, n_order,t_order)
    ...
    [Aeq, beq] = getAbeq(n_seg, n_order,t_order,waypoints, ts, start_cond, end_cond);
    f = zeros(size(Q,1),1);
