...
enum HeuristicType{
    Manhattan,Euclidean,Diagnoal,Dijkstra
};
    ...
    for(int dx=-1;dx<=1;dx++){
        for(int dy=-1;dy<=1;dy++){
            for(int dz=-1;dz<=1;dz++){
                ...
                nx = current_index(0) + dx;
                ny = current_index(1) + dy;
                nz = current_index(2) + dz;

                if(isFree(nx,ny,nz)){
                    neighborPtrSets.push_back(GridNodeMap[nx][ny][nz]);
...
double AstarPathFinder::getHeu(GridNodePtr node1, GridNodePtr node2)
{
    ...
    */
    ...
    double hScore = 0;
    HeuristicType heuristic_type_ = Manhattan;
    ...
    case Diagnoal:
        {
            ...
            hScore = dx + dy + dz + (sqrt(3.0)-3) * min_xyz;
            break;
        }
    case Dijkstra:
        {
        ...
        */         
        for(int i = 0; i < (int)neighborPtrSets.size(); i++){
            ...
            */
            ...
            double neighbor_gScore = edgeCostSets[i];
